...
export class EmotionTimelineChartData {
    ...
    private chartLabels: string[] = ['Happiness', 'Surprise', 'Sadness', 'Anger', 'Disgust', 'Fear', 'Contempt'];
    ...
    constructor(timeMarkers: Array<TimeMarker>, normalizeFactor = 1) {
        ...
        timeMarkers.forEach(marker => {
            ...
            let AddToRest = 0;
            let neutralValue = 0//this.getPercentage(marker.emotion.neutral);
            ...
            this.data[0].data.push(this.getPercentage(marker.emotion.happiness) + AddToRest);
            this.data[1].data.push(this.getPercentage(marker.emotion.surprise) + AddToRest);
            this.data[2].data.push(this.getPercentage(marker.emotion.sadness) + AddToRest);
            this.data[3].data.push(this.getPercentage(marker.emotion.anger) + AddToRest);
            this.data[4].data.push(this.getPercentage(marker.emotion.disgust) + AddToRest);
            this.data[5].data.push(this.getPercentage(marker.emotion.fear) + AddToRest);
            this.data[6].data.push(this.getPercentage(marker.emotion.contempt) + AddToRest);
